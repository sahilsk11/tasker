...
- For Tasker browser verification, prefer `pnpm dev:isolated`. It creates a temporary SQLite database, applies migrations and task action defaults, allocates API/web ports, writes a `.tasker/dev-runs/*.json` manifest, and prints the exact API/web URLs. Check `GET /api/runtime` or the manifest before claiming a Tasker session so you know which database the API is using.
